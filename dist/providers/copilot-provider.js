...
export function isMissingAuthenticationError(error) {
    const message = error instanceof Error ? error.message : String(error);
...
function buildImplementationPrompt(config, run) {
...
export class CopilotProvider {
    ...
    async runSession(run, logger, options) {
        ...
        try {
            ...
            const finalMessage = await session.sendAndWait({ prompt: options.prompt }, 30 * 60 * 1000);
    ...
    async sendPrompt(run, logger, prompt, sessionId, resumeSessionId) {
        try {
            ...
            if (!this.config.github.token && this.config.copilot.baseDirectory && isMissingAuthenticationError(error)) {
                await logger.log("Project-local Copilot auth state is empty; retrying with the user's default Copilot login.");
    ...
    async implement(run, logger) {
        const result = await this.sendPrompt(run, logger, buildImplementationPrompt(this.config, run), run.id, run.integration.copilotSessionId);
        return {
