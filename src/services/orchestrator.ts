...
function extractLastJsonBlock(output: string): string {
...
function shorten(message: string, maxLength = 140): string {
...
export class OrchestratorService {
  ...
  private readonly prd: PrdService;

  public constructor(
    private readonly config: AppConfig,
    private readonly options: { liveOutput?: boolean } = {},
  ) {
  ...
  private async runCouncilLoop(run: RunRecord, logger: RunLogger): Promise<void> {
    ...
    for (const item of items) {
      ...
      if (firstModeration.requiresFollowUp && item.maxRounds > 1) {
        ...
        const roundTwo = await this.collectContributions(
          current.id,
          item,
          2,
          followUpPrompt,
        ...
        decision = await this.moderateItem(
          current.id,
          item,
          2,
          [...roundOne, ...roundTwo],
      ...
      await this.store.updateRun(current.id, (mutableRun) => {
        ...
        nextTeam.decisions = [
          ...
          this.deliberation.createDecision(
            item,
            roundsUsed,
            decision.summary,
            roundsUsed === 1 ? decision.followUpPrompt : undefined,
          ),
        ];
        ...
        prd.discussionItems = prd.discussionItems.map((candidate) =>
    ...
    const prdResult = await this.copilot.sendPrompt(
      withDecisions,
      logger,
      prdPrompt,
      `${run.id}-prd`,
  ...
  private async collectContributions(
    ...
    for (const persona of DEVELOPER_PERSONAS) {
      ...
      const result = await this.copilot.sendPrompt(
        run,
        logger,
        prompt,
        `${run.id}-${persona.id}-${item.id}-r${round}`,
      );
      ...
      const contribution = this.deliberation.createContribution(
        item,
        persona.id,
        persona.name,
        round,
  ...
  private async moderateItem(
    runId: string,
    item: PrdDiscussionItem,
    round: 1 | 2,
    contributions: CouncilContribution[],
    logger: RunLogger,
  ): Promise<z.infer<typeof MODERATION_RESPONSE_SCHEMA>> {
    const run = await this.store.getRun(runId);
  ...
  private buildContributionPrompt(
    ...
    round: 1 | 2,
    agendaPrompt: string,
    persona: (typeof DEVELOPER_PERSONAS)[number],
  ): string {
  ...
  private buildPrdPrompt(run: RunRecord): string {
    ...
    const contributions = run.team?.contributions.map(
      (contribution) =>
        `### ${contribution.name} / ${contribution.itemTitle} / round ${contribution.round}\n${contribution.content}`,
    ) ?? [];
