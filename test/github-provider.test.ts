...
describe("GitHubProvider", () => {
  ...
  it("does not fail the run when repository_dispatch returns an error", async () => {
    const config = await createTestConfig("github-provider");
    await initRepository(config.projectRoot);
    config.github.token = "test-token";
    const provider = new GitHubProvider(config);
    const workspacePath = path.join(config.workspaceRoot, "run-2");
    await execFile("git", ["clone", "--branch", "main", "--single-branch", config.projectRoot, workspacePath]);
    ...
    const run = {
      ...
      feature: "Ship feature",
      featureSlug: "ship-feature",
      featureBranch: "copilot/ship-feature-run-2",
      workspacePath,
      ...
      workflow: {
        provider: "openspec" as const,
        changeName: "agent-ship-feature-run-2",
      },
      team: {
        orchestratorName: "Hans",
        memberResults: [],
    ...
    expect(published.detail).toContain("repository_dispatch failed (404)");
  });
});
